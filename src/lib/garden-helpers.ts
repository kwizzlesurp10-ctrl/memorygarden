...
export function calculateGrowthMetrics(memory: Memory, nearbyMemories: Memory[]): GrowthMetrics {
  ...
  const varietyModifiers = {
    ...
    vitality,
    width: Math.floor(32 + (vitality * varietyModifiers.widthMult * 0.
    foliageDensity: Math.min(100, vitality + nearbyMemories.length * 5)
    lastInteractionAt: memory.lastVisited ? new Date(memory.lastVisited
}
function getPlantStageFromMetrics(metrics: GrowthMetrics): PlantStage {
  ...
  const boostAmount = multipliers[boostLeve
  return {
    visitCount: 
}

  
      lower.includes('wonderf
  }
  i
...
export async function generateAIReflection(memory: Memory, nearbyMe
Me
${nearbyMemories.length > 0 ? `Nearby memories: ${nearbyMemories.
Write a brief, poetic reflection (2-3 sentences) that honors this m
- Connect to broader themes of growth, time, or connection if appropr
...
export function getPlantColor(emotionalTone: EmotionalTone): string 
  ...
  const sk
      dawn: 'l
      dusk: 'linear-gradient(to bottom, oklch(0.
   
 

    },
      dawn: 'linear-gradient(to bo
...
export function getSeasonalPlantModifier(season: Season, em
    spring: {
  ...
  artStyle: ArtStyle
  const stageDescriptions: Record<PlantStage, str
    sprout: 'A small sprout emerg
    young: 'A developing plant with v
    bloom: 'A flowering plant in full b
    elder: 'An ancient, majestic plant w
...
export function filterMemories
    ...
    if (searchQuery.trim()) {
      const textMatch = memory.text.
      const reflectionMatch = memory.refle
      )
    }
    if (filters.
...
export function getActiveFilterCount(searchQuery: string,
  if (searchQ
